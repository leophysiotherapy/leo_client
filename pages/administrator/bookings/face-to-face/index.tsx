...
import AppointmentQuery from '@/components/admin/appointment/facetoface/appointmentQuery'
import { useRouter } from 'next/router'

const OnlineHeader = [ "Date & Time", "Name", "Services", "Status", "Receipt", "Actions" ]
                            ...
                            toggleDate ? dataSpeficDate?.getAppointmentByDateTime.map(({ appointmentID, date, link, platform, services, status, time, patients }: any) => (
                                patients.map(({ profile }: any) => (
                                    profile.map(({ fullname }: any) => (
                                        <AppointmentQuery key={appointmentID} appointmentID={appointmentID} date={date} time={time} services={services} status={status} fullname={fullname} />
                                    ))
                                ))
                            )) :
